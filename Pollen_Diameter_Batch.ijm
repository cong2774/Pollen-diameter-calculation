...
function main() {
    ...
    File.append("Image,Particle_ID,Final_Status,Status_By_Area_Diameter,Status_By_Roundness,Status_By_Solidity,Status_By_Black_Filter,Status_By_Background_Contrast,Calibration_Mode,Threshold_Method,Area_um2,Diameter_from_area_um,Diameter_Feret_um,Circularity,Roundness,Solidity,X_um,Y_um,Black_Center_Percent,Mean_R,Mean_G,Mean_B,Background_Mean_RGB,Center_Background_Contrast,ScaleBar_px,Um_per_px", particlesPath);
    File.append("Image,Image_Status,Calibration_Mode,Threshold_Method,ScaleBar_px,Um_per_px,Valid_Count_Fertile_Black,Invalid_Count_By_Diameter,Excluded_Count_By_Roundness,Excluded_Count_By_Solidity,Excluded_Count_By_Black_Filter,Excluded_Count_By_Background_Contrast,Mean_Diameter_from_area_um,SD_Diameter_from_area_um,Min_Diameter_from_area_um,Max_Diameter_from_area_um", summaryPath);
...
function processOneImage(inputDir, outputDir, previewDir, file, imageIndex, particlesPath, summaryPath, tempPath, calibrationArg, minDiamUm, maxDiamUm, thresholdMethod, redMin, greenMax, blueMax, searchRightPercent, searchBottomPercent, minScalePx, filterArg, savePreview) {
    ...
    if (scalePx < minScalePx) {
        File.append(csv(file) + ",scale_not_found,none,none,,,0,0,0,0,0,0,,,,", summaryPath);
        close();
        return 0;
    }
    ...
    for (r = 0; r < nResults; r++) {
        ...
        if (statusAreaDiameter == "valid" && statusRoundness == "valid" && statusSolidity == "valid" && statusBlack == "black" && statusBackground == "valid") {
            finalStatus = "valid";
            validCount++;
        ...
        File.append(csv(file) + "," + (r + 1) + "," + finalStatus + "," + statusAreaDiameter + "," + statusRoundness + "," + statusSolidity + "," + statusBlack + "," + statusBackground + "," + calibrationMode + "," + actualThresholdMethod + "," + d2s(area, 3) + "," + d2s(areaDiameter, 3) + "," + d2s(feret, 3) + "," + d2s(circularity, 3) + "," + d2s(roundness, 3) + "," + d2s(solidity, 3) + "," + d2s(x, 3) + "," + d2s(y, 3) + "," + d2s(blackCenterPercent, 3) + "," + d2s(meanR, 1) + "," + d2s(meanG, 1) + "," + d2s(meanB, 1) + "," + d2s(backgroundMean, 1) + "," + d2s(backgroundContrast, 1) + "," + d2s(scalePx, 3) + "," + d2s(umPerPx, 6), particlesPath);
    }
    ...
    if (validCount > 0) {
        ...
        if (validCount > 1)
            ...
            sdDiam = 0;
        File.append(csv(file) + ",ok," + calibrationMode + "," + actualThresholdMethod + "," + d2s(scalePx, 3) + "," + d2s(umPerPx, 6) + "," + validCount + "," + invalidDiameterCount + "," + invalidRoundnessCount + "," + invalidSolidityCount + "," + nonblackCount + "," + backgroundInterferenceCount + "," + d2s(meanDiam, 3) + "," + d2s(sdDiam, 3) + "," + d2s(minDiam, 3) + "," + d2s(maxDiam, 3), summaryPath);
    } else {
        File.append(csv(file) + ",no_valid_particles," + calibrationMode + "," + actualThresholdMethod + "," + d2s(scalePx, 3) + "," + d2s(umPerPx, 6) + ",0," + invalidDiameterCount + "," + invalidRoundnessCount + "," + invalidSolidityCount + "," + nonblackCount + "," + backgroundInterferenceCount + ",,,,", summaryPath);
    }
